	...
	public Table(ArrayList<Account> players) {
		...
		playedCards.addListener((o, OldValue, NewValue) -> {
			if (NewValue.intValue() > 1) {
				// Artificial break in order to give the clients time to process previous
				// messages
				try {
					Thread.sleep(1000);
...
//		super();
	 ...
	 */
	// David: Gibt bei den ersten zwei Aufrufen 13 Karten zurück und beim dritten
	// Aufruf die restlichen 26 Karten
	public void deal() {
		...
		for (int i = 0; i < 3; i++) {
			if (deck.getCardsRemaining() > 26) {
				for (int j = 0; j < 13; j++) {
					card = deck.dealCard();
					if (i == 0)
						players.get(0).addHandCard(card);
					else
						players.get(1).addHandCard(card);
				}
			} else {
				for (int j = 0; j < 26; j++) {
					card = deck.dealCard();
	...
	// David: Wertet den Sieger des Spiels aus am Ende aller Runden aus und gibt den
	// Accountnamen des Siegers zurück.
	public String winner() {
		String win = "NoWinner";
		gameWinner();
		if (fractionPointsP1 >= 3)
			return players.get(0).getUsername();
		if (fractionPointsP2 >= 3)
			return players.get(1).getUsername();
		return win;
	}

	/*
	 * David: Sortiert die gewonnenen Karten in die verschiedenen Fraktionen. Danach
	 * wird die Sieger Evaluierung und das Hinzufügen der eventuellen Siegerpunkte
	 * aufgerufen
	 */
	...
	/*
	 * David: Vergleicht die Anzahl Anhänger einer Fraktion und gibt den Spieler der
	 * gewonnen hat zurück. Im Fall, das beide Spieler gleich viele Karten haben
	 * wird ausgewertet wer die höchste hat. Der ist dann auch Sieger dieser
	 * Fraktion. Falls z.B: beide keine Karte von der selben Fraktion hat gibt es
	 * keinen Sieger.
	 */
	...
	// David: Fügt dem Sieger einer Fraktion einen Punkt hinzu, falls es kein
	// Unentschieden gibt.
	...
	// David: Sortiert die Karten und gibt die letzte Karte, welche die Höchste ist
	// zurück
	...
	// DS
	/*
	 * David: Die Methode ermittelt den Gewinner am Ende jeder Runde aus den zwei
	 * Spielern. Sie löst die Übermittelung an die Clients mit der Rückmeldung aus.
	 */
		...
		if (secondRoundStarted) {
			if (players.get(0).getPlayedCard().getSuit().toString().equals("dwarf")
				...
				case 1:
					...
					break;
				case 2:
					playedCardString = dwarfs.get(0).toString() + "|" + dwarfs.get(1).toString();
					break;
				}
				...
				playedCardString = players.get(0).getPlayedCard().toString() + "|"
						+ players.get(1).getPlayedCard().toString();
	...
	// David: Evaluiert welche Karte gewonnen hat und gibt je nach Kartenposition 1
	// oder -1 zurück
	private int evaluateWinCard(Card cardP1, Card cardP2) {
		...
		if (suitToString(cardP1).equals("goblin") && suitToString(cardP2).equals("knight")
			...
			if (suitToString(cardP1).equals("knight"))
				return 1;
			else
				return -1;
		}
	...
	private void addFollowerCards(int win, Card tableC, Card nextTC) {
		switch (win) {
		case 1:
			players.get(0).getFollowerCards().add(tableC);
			players.get(1).getFollowerCards().add(nextTC);
			break;
		case -1:
			players.get(1).getFollowerCards().add(tableC);
			players.get(0).getFollowerCards().add(nextTC);
			break;
		}
	}

	/*
	 * David: Fügt in Runde 2 jedem Spieler die gewonnenen Karten hinzu, falls ein
	 * Zwerg gespielt wurde. Falls noch eine Untotenkarte dabei ist, wird die
	 * separat zu den Untotenkarten hinzugefügt.
	 */
	private void addFollowerCards(int win, ArrayList<Card> dwarfs) {
		if (dwarfs.size() == 2) {
			switch (win) {
			case 1:
				for (Card dwarf : dwarfs) {
					players.get(1).getFollowerCards().add(dwarf);
				}
				break;
			case -1:
				for (Card dwarf : dwarfs) {
					players.get(0).getFollowerCards().add(dwarf);
				}
				break;
			}
		} else {
			...
			case 1:
				...
				players.get(1).getFollowerCards().add(dwarfs.get(0));
				break;
			...
			case -1:
				...
				players.get(0).getFollowerCards().add(dwarfs.get(0));
				break;
			}
		}
	}
	...
	/*
	 * David: Fügt in Runde 2 dem Sieger beide gewonnenen Karten hinzu, sofern kein
	 * Zwerg gespielt wurde. Die Untoten werden separat zu den Untoten hinzugefügt
	 * und nicht zu den normalen Followerkarten.
	 */
	private void addFollowerCards(int win) {
		if (players.get(0).getPlayedCard().getSuit().toString().equals("undead")
			...
			Boolean one = false;
			if (players.get(0).getPlayedCard().getSuit().toString().equals("undead"))
				tmpUndeads.add(players.get(0).getPlayedCard());
	...
	// David: Die Untoten werden im Fall das sie vorhanden sind den Untotenkarten
	// hinzugefügt
	private void addUndeads(Card cardP1, Card cardP2, int win) {
		...
		if (suitToString(cardP1).equals("undead") || suitToString(cardP2).equals("undead")) {
			switch (win) {
			case 1:
				if (suitToString(cardP1).equals("undead")) {
					players.get(0).addUndeadCard(cardP1);
					tmpUndeads.add(cardP1);
				}
				if (suitToString(cardP2).equals("undead")) {
					players.get(0).addUndeadCard(cardP2);
					tmpUndeads.add(cardP2);
				}
				break;
			case -1:
				if (suitToString(cardP1).equals("undead")) {
					players.get(1).addUndeadCard(cardP1);
					tmpUndeads.add(cardP1);
				}
				if (suitToString(cardP2).equals("undead")) {
					players.get(1).addUndeadCard(cardP2);
					tmpUndeads.add(cardP2);
				}
				break;
			}
		...
		case 0:
			break;
		case 1:
			undeadString = tmpUndeads.get(0).toString();
			break;
		case 2:
			undeadString = tmpUndeads.get(0).toString() + "|" + tmpUndeads.get(1).toString();
			break;
		}
	}

	// David: Wandelt die Karte in einen String und gibt nur den suit der Karte als
	// String zurück
	...
	// Jannick: calls the winner() method and sends a new message with the winner to
	// both clients
